...
public class ComponentRCVillageHouse extends ComponentVillage
{
	...
	protected ComponentRCVillageHouse(ComponentVillageStartPiece startPiece, int type, Random random, StructureBoundingBox _boundingBox, int direction){
	...
	public static Object buildComponent(ComponentVillageStartPiece startPiece, List pieces, Random random, int x, int y, int z, int direction, int type) {
        ...
        if(canVillageGoDeeper(_boundingBox)){ 
        	if(StructureComponent.findIntersecting(pieces, _boundingBox) == null){
        		return new ComponentRCVillageHouse(startPiece, type, random, _boundingBox, direction);
        	}
        }
		return null;
	}
