...
public class WorldGenMinableMeta extends WorldGenerator
{
  ...
  public WorldGenMinableMeta(int blockid, int meta, int numberOfBlocks)
  {
  ...
  public boolean generate(World world, Random random, int i, int j, int k)
  {
    ...
    double d = i + 8 + MathHelper.sin(randomBase) * this.numberOfBlocks / 8.0F;
    double d1 = i + 8 - MathHelper.sin(randomBase) * this.numberOfBlocks / 8.0F;
    double d2 = k + 8 + MathHelper.cos(randomBase) * this.numberOfBlocks / 8.0F;
    double d3 = k + 8 - MathHelper.cos(randomBase) * this.numberOfBlocks / 8.0F;
          ...
          for (int targetY = yStart; targetY <= yEnd; targetY++) {
            ...
            if (d12 * d12 + d13 * d13 < 1.0D)
            {
              for (int targetZ = zStart; targetZ <= zEnd; targetZ++) {
                ...
                if ((d12 * d12 + d13 * d13 + d14 * d14 < 1.0D) && (world.getBlockId(targetX, targetY, targetZ) == Block.stone.blockID)) {
                  world.setBlock(targetX, targetY, targetZ, this.mineableBlockId, this.mineableBlockMeta, 2);
                  hasGenerated = true;
                }
              }
            }
